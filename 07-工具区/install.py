...
REQ_FILE = PROJECT_ROOT / "01-配置区" / "requirements.txt"
CONFIG_TEMPLATE = PROJECT_ROOT / "01-配置区" / "config.json.template"
CONFIG_FILE = PROJECT_ROOT / "01-配置区" / "config.json"
README_FILE = PROJECT_ROOT / "01-配置区" / "README_安装说明.md"
...
def create_mac_launcher():
    cmd_path = PROJECT_ROOT / "01-配置区" / "启动书童.command"
    content = f"""#!/bin/bash
# 伴读书童AI 一键启动器（Mac）
cd "{PROJECT_ROOT}"
"{VENV_DIR / 'bin' / 'python'}" 01-配置区/start.py
"""
...
def create_win_launcher():
    bat_path = PROJECT_ROOT / "01-配置区" / "启动书童.bat"
    content = f"""@echo off
REM 伴读书童AI 一键启动器（Windows）
cd /d "{PROJECT_ROOT}"
"{VENV_DIR / 'Scripts' / 'python.exe'}" 01-配置区/start.py
pause
"""
...
def main():
    ...
    if sys.version_info < (3, 12):
        print("[错误] 需要 Python 3.12 或更高版本，请访问 python.org 下载")
        sys.exit(1)
    ...
    print("  2. 双击运行启动器")
    if platform.system() == "Windows":
        print("  启动器：01-配置区/启动书童.bat")
    else:
        print("  启动器：01-配置区/启动书童.command")
    print("=" * 50)
