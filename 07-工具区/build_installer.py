...
BUILD_DIR = PROJECT_ROOT / "05-交付区" / "产品交付" / "安装包"
ARCHIVE_ZONE = PROJECT_ROOT / "04-工作区" / "档案区" / "家庭群"

EXCLUDES = {
    # 版本控制和环境
    ".git", ".venv", "__pycache__", ".DS_Store",
    # 临时和归档
    "05-交付区/临时交付", "08-归档区", "04-工作区/日志", "05-交付区/产品交付",
    # 灵魂文件：只存云端，不进安装包
    "AGENTS.md",
    "WORKFLOW.md",
    "03-引擎区/书童程序/数据/提示词",
    ...
    "00-灵魂区/目录说明.md",
    "setup.py",
    "01-配置区/config.json",  # 每安装包单独生成
    # 云端服务端文件（只部署在云端，不进客户端安装包）
    "06-对接区/cloud_server.py",
    "01-配置区/启动云端书童.command",
    "06-对接区/前端页面/云端师父控制台.html",
    "04-工作区/云端数据区",
    # 师父专属入口与工具（家庭安装包不应包含）
    "06-对接区/前端页面/师父PC端.html",
    "03-引擎区/static/师父头像.jpg",
    "设置书童头像.command",
    "解锁核心文件.command",
    "锁定核心文件.command",
    # 内部开发/打包/部署文件
    ...
    # 测试、演示、工具脚本（非家庭用户所需）
    "06-对接区/前端页面/测试页面.html",
    "06-对接区/前端页面/书童形象展示.html",
...
REQUIRED_FILES = [
    "01-配置区/config.json.template",
    "01-配置区/README_安装说明.md",
    "07-工具区/install.py",
    "01-配置区/start.py",
    "01-配置区/requirements.txt",
]
...
def copy_family_data(dest: Path, family_id: str, family_dir: Path):
    ...
    dest_family_dir = dest / "04-工作区" / "档案区" / "家庭群" / Path(*rel_parts)
...
def create_config(dest: Path, family_id: str, cloud_api_base: str = None, api_key: str = None):
    ...
    config_path = dest / "01-配置区" / "config.json"
...
def create_family_readme(dest: Path, family_id: str, family_name: str):
    """为家庭安装包生成定制说明"""
    readme_path = dest / "01-配置区" / "README_安装说明.md"
    readme_path.parent.mkdir(parents=True, exist_ok=True)
    base_readme = PROJECT_ROOT / "01-配置区" / "README_安装说明.md"
...
def cleanup_files_by_package_type(package_dir: Path, package_type: str):
    """根据安装包类型清理不该出现的文件（路径已对齐九区结构）"""
    always_remove = [
        "06-对接区/前端页面/师父PC端.html",
        "03-引擎区/static/师父头像.jpg",
        "01-配置区/设置书童头像.command",
        "01-配置区/解锁核心文件.command",
        "01-配置区/锁定核心文件.command",
        "01-配置区/启动感官系统测试.command",
        "06-对接区/前端页面/测试页面.html",
        "06-对接区/前端页面/书童形象展示.html",
        "07-工具区/build_installer.py",
        "00-灵魂区/云端与本地边界说明.md",
    ]
    type_specific_remove = {
        "child": ["06-对接区/前端页面/墨童.html"],
        "mobile": [
            "06-对接区/前端页面/墨童.html",
            "06-对接区/前端页面/书童家庭端.html",
        ],
    }
    ...
    for rel_name in to_remove:
        ...
        if target.exists():
            try:
                ...
                print(f"[警告] 清理 {rel_name} 失败: {e}")


def apply_package_type(package_dir: Path, package_type: str, family_id: str):
    """根据安装包类型裁剪入口与默认启动行为"""
    entry_path = package_dir / "06-对接区" / "前端页面" / "书童家庭访问入口.html"
...
def _cleanup_other_families(package_dir: Path, keep_family_id: str):
    ...
    local_family_dir = package_dir / "03-引擎区" / "书童程序" / "数据" / "家庭"
...
def main():
    ...
    parser.add_argument("--cloud-api-base", type=str, default=os.environ.get("BOOKBOY_CLOUD_API_BASE", "https://bookkidai.com"), help="云端 API 地址")
