...
class CloudSyncManager:
    ...
    def __init__(self,
        ...
        """
        Args:
            archive_dir: 本地档案区根目录，默认 项目根目录/档案区/家庭群
            cloud_dir: 云端数据区根目录，默认 项目根目录/云端数据区
            salt: 哈希盐值
        """
        project_root = Path(__file__).resolve().parents[2]

        self.archive_dir = Path(archive_dir) if archive_dir else project_root / "档案区" / "家庭群"
        self.cloud_dir = Path(cloud_dir) if cloud_dir else project_root / "云端数据区"
