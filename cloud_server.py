...
# 云端音频缓存目录
AUDIO_CACHE_DIR = PROJECT_ROOT / "书童程序" / "数据" / "语音缓存"
...
SOUL_FILES = {
    ...
    "system_prompt": PROJECT_ROOT / "书童程序" / "数据" / "提示词" / "系统提示词整合版_可运行.md",
    "master_prompt": PROJECT_ROOT / "书童程序" / "数据" / "提示词" / "师父模式系统提示词.md",
}
...
def init_ability_engines():
    ...
    # 确保必要目录存在
    data_dir = PROJECT_ROOT / "书童程序" / "数据"
    archive_dir = PROJECT_ROOT / "档案区"
...
def cloud_parent_creations():
    ...
    try:
        family_dir = get_family_dir(family_id) if get_family_dir else PROJECT_ROOT / "云端数据区" / "家长创作" / family_id
        family_dir.mkdir(parents=True, exist_ok=True)
...
MASTER_CONSOLE_HTML = PROJECT_ROOT / "云端师父控制台.html"
