...
运行方式：
  正常模式（师父手动操作）：
    .venv/bin/python 工具脚本/更新核心文件校验.py
  
  自动模式（书童代操作，测试用）：
    .venv/bin/python 工具脚本/更新核心文件校验.py --auto
"""
...
try:
    ...
    print("请使用项目虚拟环境运行：")
    print("  .venv/bin/python 工具脚本/更新核心文件校验.py")
    sys.exit(1)
...
def authenticate_master(project_root, auto_mode=False, auto_password=None, no_camera=False):
    ...
    auth_dir = project_root / "书童程序" / "数据" / "认证记录"
    auth_config_path = project_root / "书童程序" / "数据" / "师父认证配置.json"
