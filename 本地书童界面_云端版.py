...
FAMILY_DATA_DIR = PROJECT_ROOT / "书童程序" / "数据" / "家庭"
...
class BookBoyCloudHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 家庭访问入口
        if path == "/entry":
            entry_path = PROJECT_ROOT / "书童家庭访问入口.html"
        ...
        if m:
            ...
            mode = m.group(2)
            html_path = PROJECT_ROOT / "书童家庭端.html"
            if html_path.exists():
                content = html_path.read_text(encoding="utf-8")
        ...
        if m:
            family_id = m.group(1)
            html_path = PROJECT_ROOT / "书童手机端.html"
            if html_path.exists():
        ...
        if m:
            family_id = m.group(1)
            html_path = PROJECT_ROOT / "师父PC端.html"
            if html_path.exists():
                content = self._inject_family_id(html_path, family_id)
                self.send_response(200)
                self.send_header("Content-Type", "text/html; charset=utf-8")
        ...
        if m:
            ...
            html_path = PROJECT_ROOT / "墨童.html"
            if html_path.exists():
                content = self._inject_family_id(html_path, family_id)
                self.send_response(200)
                self.send_header("Content-Type", "text/html; charset=utf-8")
...
def main():
    ...
    server = ThreadingHTTPServer((HOST, PORT), BookBoyCloudHandler)
