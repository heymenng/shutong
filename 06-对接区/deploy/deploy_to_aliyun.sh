...
cp "$PROJECT_ROOT/06-对接区/cloud_server.py" "$LOCAL_TMP/"
cp "$PROJECT_ROOT/06-对接区/前端页面/云端师父控制台.html" "$LOCAL_TMP/"
cp "$PROJECT_ROOT/01-配置区/requirements_cloud.txt" "$LOCAL_TMP/requirements.txt"
cp "$PROJECT_ROOT/00-灵魂区/AGENTS.md" "$LOCAL_TMP/"

# 云端核心模块
mkdir -p "$LOCAL_TMP/书童程序/核心"
cp "$PROJECT_ROOT/03-引擎区/书童程序/__init__.py" "$LOCAL_TMP/书童程序/"
cp "$PROJECT_ROOT/03-引擎区/书童程序/配置.py" "$LOCAL_TMP/书童程序/"
cp "$PROJECT_ROOT/03-引擎区/书童程序/核心/__init__.py" "$LOCAL_TMP/书童程序/核心/"
cp "$PROJECT_ROOT/03-引擎区/书童程序/核心/语言模型.py" "$LOCAL_TMP/书童程序/核心/"
cp "$PROJECT_ROOT/03-引擎区/书童程序/核心/语音模块.py" "$LOCAL_TMP/书童程序/核心/"
cp "$PROJECT_ROOT/03-引擎区/书童程序/核心/讯飞语音.py" "$LOCAL_TMP/书童程序/核心/"
cp "$PROJECT_ROOT/03-引擎区/书童程序/核心/讯飞超拟人语音.py" "$LOCAL_TMP/书童程序/核心/"

# 提示词
mkdir -p "$LOCAL_TMP/书童程序/数据/提示词"
cp "$PROJECT_ROOT/03-引擎区/书童程序/数据/提示词/系统提示词整合版_可运行.md" "$LOCAL_TMP/书童程序/数据/提示词/"
cp "$PROJECT_ROOT/03-引擎区/书童程序/数据/提示词/师父模式系统提示词.md" "$LOCAL_TMP/书童程序/数据/提示词/"
