...
def read_core_files():
    ...
    if missing:
        ...
        for f in missing:
            error_msg += f"  - {f}\n"
        error_msg += "\n请确保：\n"
        error_msg += "1. 00-灵魂区/AGENTS.md 存在（道统核心）\n"
        error_msg += "2. 00-灵魂区/WORKFLOW.md 存在（运行手册）\n"
        print(error_msg, file=sys.stderr)
        sys.exit(1)
...
def negentropy_self_check(agents_content, workflow_content):
    ...
    # 3. R1安全协议是否存在
    r1_path = PROJECT_ROOT / "03-引擎区" / "书童程序" / "数据" / "R1安全协议_V1.0.py"
    if r1_path.exists():
    ...
    guardian_path = PROJECT_ROOT / "03-引擎区" / "书童程序" / "核心" / "书童守护.py"
    ...
    product_dir = PROJECT_ROOT / "04-工作区" / "项目文档" / "产品资料"
    hype_found = []
    anxiety_found = []
